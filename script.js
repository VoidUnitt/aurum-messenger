...
document.getElementById('registerBtn').addEventListener('click', async () => {
    ...
    if (!name || !email || !password) {
        authMessage.innerHTML = '<div class="error">❌ Заполните все поля</div>';
        return;
    }
    ...
    try {
        ...
        authMessage.innerHTML = '<div class="success">✅ Регистрация успешна! Теперь войдите.</div>';
        document.getElementById('regName').value = '';
        document.getElementById('regEmail').value = '';
        document.getElementById('regPassword').value = '';
        document.querySelector('.tab[data-tab="login"]').click();
    } catch (error) {
        authMessage.innerHTML = `<div class="error">❌ ${error.message}</div>`;
    }
});
...
document.getElementById('loginBtn').addEventListener('click', async () => {
    ...
    if (!email || !password) {
        authMessage.innerHTML = '<div class="error">❌ Введите email и пароль</div>';
        return;
    }
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        document.getElementById('userAvatar').textContent = (userData?.name || user.displayName)[0];
...
// ========== СОЗДАТЬ ДЕМО-ЧАТ ==========
// Добавляем кнопку для создания тестового чата
setTimeout(() => {
    const sidebar = document.querySelector('.sidebar');
    if (sidebar && !document.querySelector('.demo-chat-btn')) {
        ...
        document.querySelector('.search-box').after(demoBtn);
    }
}, 1000);
...
async function createDemoChat() {
    ...
    // Добавляем в чаты пользователя
    await updateDoc(doc(db, 'users', currentUser.uid), {
        chats: arrayUnion(demoChatId)
    });
    
    loadChats();
    alert('✅ Создан демо-чат! Нажмите на него чтобы начать общение.');
}
